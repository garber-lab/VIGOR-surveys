...
data_dict_clean <- dict %>%
  rename(
    ...
    result_readable = `result readable`,
    possible_answers = `possible answers`
  )

# display summary of surveys
dict %>%
  group_by(survey_type, survey_topic, surveyname, survey_display_name) %>%
  summarize(
    n_questions = n_distinct(resultidentifier),
    .groups = "drop"
  )
...
surveys_enriched <- surveys_clean %>%
  ...
  mutate(
    ...
    # # expected survey wave based on 
    # survey_wave_exp = case_when(
    #   survey_type == "BL" ~ "Baseline",
    #   days_since_enrollment <= 120 ~ "3-Month", 
    #   days_since_enrollment <= 210 ~ "6-Month",
    #   days_since_enrollment <= 300 ~ "9-Month",
    #   days_since_enrollment <= 390 ~ "12-Month",
    #   TRUE ~ "Later"
    # ),
    # # Create readable survey period
    # survey_period = paste0(survey_wave, " (", survey_type, ")")
  ) %>%
  # Filter out rows without dictionary matches (administrative surveys)
  filter(!is.na(survey_topic))  %>%
  # Select key columns for analysis
  select(
    patient_id, FamilyID, fam_pos, vit_status_exp,
    recorded_date, recorded_dttm, enroll_date,
    days_since_enrollment, 
    # survey_wave_exp, survey_period,
    survey_topic, survey_display_name, survey_type,
    step_readable, result_readable, 
    answer_clean, possible_answers,
