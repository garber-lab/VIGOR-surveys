...
data_dict_clean <- data_dictionary %>%
  rename(
  ...
  # normalize question text for Ad Hoc questions
  mutate(
    question_standardized = case_when(
      resultidentifier == "new_spots_adhoc" ~ "Have you noticed any new white spots on your skin that you think can be vitiligo or improvement (repigmentation) of your vitiligo?",
      resultidentifier == "new_spots_duration" ~ "How long ago did you notice your new white spots?",
      resultidentifier == "vitiligo_location1_adhoc" ~ "Where is your vitiligo located? Please include the new spot you're currently reporting.\n\n*Select all that apply*",
      .default = question_readable
    )
  ) %>%
  # create a comprehensive lookup key including version
  select(surveyname, surveyversion, stepidentifier, resultidentifier,
         survey_type, survey_topic, step_readable,
         question_standardized, 
         # question_readable,
         possible_answers)
...
survey_summary_no_version <- data_dict_clean %>%
...
surveys_enriched_stress <- surveys_clean %>%
  ...
  arrange(patient_id, recorded_dttm, question_standardized) %>%
  # remove any duplicate records from the same patient on the same day 
  distinct(patient_id, recorded_date, question_standardized, .keep_all = T) %>%
  # update survey type
  mutate(
  ...
  mutate(
    ...
    stress_score_v2 = case_when(
      str_extract(question_standardized, "^\\d+") %in% as.character(reverse_qs) ~ fct_recode(
        stress_score,
        `0` = "4",
        `1` = "3",
        `2` = "2",
